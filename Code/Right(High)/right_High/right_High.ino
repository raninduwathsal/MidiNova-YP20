...
const int NUM_KEYS = 46; // Change according to your keyboard
const int NUM_LEDS = 34; // Match to the available number of LEDs
...
const float baseBrightnessFactor = 0.05; // 5% brightness for default state
CRGB leds[NUM_LEDS];

// Pin definitions (12 keys)
const int keyPins[12] = {16, 17,5, 18, 19, 21, 22, 23, 36, 39, 34, 35};   // Key input pins array (12 keys)
const int lowerRailPin[4] = {12,14,25,4};  // Lower rail output pins (3 rails for 12 keys)
const int topRailPin[4] = {13,27,26,15};    // Top rail output pins (3 rails for 12 keys)
const int firstKeyCorrection = 40+32; //change this number to get the correct pitch note for your desired keyboard this changes by matrix

bool keyPressed[4][12] = {false};       // Track if each key is pressed
unsigned long pressStartTime[4][12];    // Store the time when each key first touches the lower rail
unsigned long pressTime[4][12];         // Store the time difference from lower to top rail for each key
...
void loop() {
    // Iterate through the rails (4 rails for 12 keys)
    ...
    ; rail++) {
        ...
        for (int key = 0; key < 12; key++) {
            int keyIndex = rail * 12 + key;  // Calculate the key index (0-11)
            int ledIndex = map(keyIndex, 0, NUM_KEYS - 1, 0, NUM_LEDS - 1); // Map keys to
